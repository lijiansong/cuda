...
// use CUDA to compute the rank of a set of webpages
// given a link graph, also known as a graph where each node is a webpage,
// and each edge is a link from one page to another.
// use the [Pagerank algorithm](http://en.wikipedia.org/wiki/Pagerank),
